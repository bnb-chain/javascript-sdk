...
var encodeAddress = function encodeAddress(value) {
  var prefix = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "cosmos";
...
var generateSignature = function generateSignature(hex, privateKey) {
  ...
  var sig = ecc.sign(msgHashHex, Buffer.from(privateKey, 'hex')); // const r = toDER(Buffer.from(sig.slice(0, 32), 'hex'));
...
var generateKeyStore = function generateKeyStore(privateKey, password) {
  ...
  var derivedKey = _cryptoBrowserify.default.pbkdf2Sync(new Buffer(password), salt, kdfparams.c, kdfparams.dklen, 'sha256');
...
var getPrivateKeyFromKeyStore = function getPrivateKeyFromKeyStore(keystore, password) {
  ...
  var derivedKey = _cryptoBrowserify.default.pbkdf2Sync(new Buffer(password), new Buffer(kdfparams.salt, 'hex'), kdfparams.c, kdfparams.dklen, 'sha256');
  ...
  var decipher = _cryptoBrowserify.default.createDecipher(json.crypto.cipher, derivedKey.slice(0, 16), new Buffer(json.crypto.cipherparams.iv, 'hex'));
